        ...
        if self.stride > 1:
            if use_adapt:
                self.branch1 = nn.Sequential(
                    approxNN.AdaPT_Conv2d(inp, inp, kernel_size=3, stride=self.stride, padding=1, groups=inp, bias=False, axx_mult=axx_mult_global),
                    nn.BatchNorm2d(inp),
                    approxNN.AdaPT_Conv2d(inp, branch_features, kernel_size=1, stride=1, padding=0, bias=False, axx_mult=axx_mult_global),
        ...
        if use_adapt:
            self.branch2 = nn.Sequential(
                approxNN.AdaPT_Conv2d(
                    ...
                    stride=1,
                    padding=0,
                    bias=False, axx_mult=axx_mult_global,
                ),
                nn.BatchNorm2d(branch_features),
                nn.ReLU(inplace=True),
                approxNN.AdaPT_Conv2d(branch_features, branch_features, kernel_size=3, stride=self.stride, padding=1, groups=branch_features, bias=False, axx_mult=axx_mult_global),
                nn.BatchNorm2d(branch_features),
                approxNN.AdaPT_Conv2d(branch_features, branch_features, kernel_size=1, stride=1, padding=0, bias=False, axx_mult=axx_mult_global),
                nn.BatchNorm2d(branch_features),
                nn.ReLU(inplace=True),
            )
        else:
    ...
    def __init__(
        ...
        if use_adapt:
            self.conv1 = nn.Sequential(
                approxNN.AdaPT_Conv2d(input_channels, output_channels, 3, 2, 1, bias=False, axx_mult=axx_mult_global),
        ...
        if use_adapt:
            self.conv5 = nn.Sequential(
                approxNN.AdaPT_Conv2d(input_channels, output_channels, 1, 1, 0, bias=False, axx_mult=axx_mult_global),
                nn.BatchNorm2d(output_channels),
                nn.ReLU(inplace=True),
            )
        else:
...
def shufflenet_v2_x1_0(pretrained: bool = False, progress: bool = True , axx_mult = 'mul8s_acc', **kwargs: Any) -> ShuffleNetV2:
    """
    ...
    if pretrained:
        ...
        state_dict = torch.load(
            script_dir + "/state_dicts/" + "shufflenet_v2_imagenet.pth", map_location='cpu'
        )
        model.load_state_dict(state_dict)
    return model
