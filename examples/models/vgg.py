...
def make_layers(cfg, batch_norm=False):
    ...
    for v in cfg:
        if v == "M":
            ...
            if use_adapt:
                conv2d = approxNN.AdaPT_Conv2d(in_channels,
                    v,
                    kernel_size=3,
                    stride=1,
                    padding=1,
                    bias=True, axx_mult=axx_mult_global)
            else:
...
def vgg11_bn(pretrained=False, progress=True, device="cpu", **kwargs):
    ...
    return _vgg("vgg11_bn", "A", True, pretrained, progress, device, axx_mult = 'mul8s_acc', **kwargs)
