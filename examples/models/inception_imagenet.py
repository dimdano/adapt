    ...
    def __init__(
        ...
        if init_weights:
            for m in self.modules():
                if isinstance(m, nn.Conv2d) or isinstance(m, approxNN.AdaPT_Conv2d) or isinstance(m, nn.Linear):
...
class BasicConv2d(nn.Module):
    def __init__(self, in_channels: int, out_channels: int, **kwargs: Any) -> None:
        super().__init__()
        if use_adapt:
            self.conv = approxNN.AdaPT_Conv2d(in_channels, out_channels, bias=False, axx_mult=axx_mult_global, **kwargs)
        else:     
...
def inception_v3(pretrained: bool = False, progress: bool = True, axx_mult = 'mul8s_acc', **kwargs: Any) -> Inception3:
    ...
    if pretrained:
        ...
        state_dict = torch.load(
            script_dir + "/state_dicts/" + "inception_v3_imagenet.pth", map_location='cpu'
        )      
